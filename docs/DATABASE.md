...
| `ai_conversations` | id; user_id → users; title; model; timestamps; deleted_at? | **Implemented Phase 8:** I(user_id, updated_at); user CASCADE on physical purge. Models are server-configured in this phase. |
...
| `ai_messages` | id; conversation_id → ai_conversations; role (`user`, `assistant`); content LONGTEXT; token counts?; created_at | **Implemented Phase 8:** I(conversation_id, created_at); conversation CASCADE on physical purge. Browser cannot insert assistant messages. |
