...
Status: **target design with Phases 1–3 and Phases 5–13 migrated; Phase 4 required no migration**. Create only the tables needed by the approved implementation phase. Remaining sections describe the future logical schema; framework/package details are resolved against locked versions when installed.

Phase 1 implemented Laravel's user/password-reset/session/cache/queue baseline. Phase 2 extended users and added social identity, OTP challenge and account-audit records. Phase 3 added Spatie's normalized role/permission tables. Phase 5 added categories. Phase 6 added `prompts`, tags, audience pivots and `prompt_uses`. Phase 7 activates personal prompt records and adds `prompt_favorites`. Phase 8 adds AI conversations, messages, requests and AI prompt-use links. Phase 9 adds conversation activity/archive metadata and per-message model attribution. Phase 10 adds logical Brand Guidelines, immutable private versions and an exact AI-request context reference. Phase 11 adds core events, bilingual categories, audience grants and redacted activity. Phase 12 adds end-time and category/start indexes for bounded calendar queries; it adds no tables. Phase 13 implements the event_files metadata table with private storage keys, fixed categories, captions, order, scan state, uploader/event foreign keys and soft deletes. Phase 14 implements logical event reports and immutable report versions with scoped report/file foreign keys, title/notes per version, soft deletion and indexed history. Uploader and file metadata are derived from event_files. Phase 15 adds six unique event/type/language communication slots, immutable revisions and a separate encrypted AI generation ledger with scoped event FK, exact Brand version, usage metadata and per-user idempotency. Notification and analytics tables remain uncreated. See the phase reports for the applied migration inventory; the remaining schema below is a future target.
...
Phase 11 implementation follows the user's narrowed core scope. `event_categories` uses normalized English/Arabic translation rows, `display_order`, active state and unique slugs. `events` includes unique slug, nullable category (SET NULL), organizer (RESTRICT), creator/updater (SET NULL), UTC DATETIME start/end with a CHECK interval constraint, timezone, status, visibility and soft deletion. Indexes cover interval, status/start, visibility/start and organizer/start. The two audience pivots below are implemented; attendee/manager delegation tables remain future design only. `event_activities` is implemented with event/actor, action, redacted JSON metadata and timestamp; its UI is deferred. The broader target fields below are not all implemented; see EVENTS.md and migrations for the current schema.
...
| Table | Main columns | Constraints / lifecycle |
| --- | --- | --- |
| `event_files` (Phase 13 implemented) | id; event_id → events; category (`photos`, `reports`, `communications`, `designs`, `other`); original_name; storage_disk/path; extension; mime_type; file_size; caption?; display_order; scan_status; uploaded_by? → users; timestamps; deleted_at | U(storage_path); I(event_id, category, deleted_at, display_order, id); event RESTRICT, uploader SET NULL. One metadata row per physical file; photo gallery filters this table. No speculative stored_files table. Phase 14 adds U(event_id, id) for scoped version references. |
| `event_reports` (Phase 14 implemented) | id; event_id → events; type (`PRE_EVENT`, `POST_EVENT`); created_by? → users; timestamps; deleted_at | U(event_id, type); U(event_id, id); event RESTRICT until explicit workspace purge; creator SET NULL. One logical report of each type; versions below. |
| `event_report_versions` (Phase 14 implemented) | id; event_id; event_report_id; event_file_id; version_number; title; notes?; created_at; deleted_at | U(event_report_id, version_number); U(event_file_id); composite FK(event_id, event_report_id) → event_reports(event_id, id); composite FK(event_id, event_file_id) → event_files(event_id, id); RESTRICT both; uploader derived from EventFile. I(event_report_id, deleted_at, version_number). |
| `event_communications` (Phase 15 implemented) | id; event_id → events; type; language (`ar`, `en`); title; content?; status (`draft`, `ready`, `approved`, `used`); revision_number; created_by?; updated_by?; archived_at?; timestamps | U(event_id,type,language); U(event_id,id); event RESTRICT; actors SET NULL. Status is manual tracking, never external delivery. |
| `event_communication_revisions` (Phase 15 implemented) | id; event_communication_id; version_number; title; content?; status; origin (`manual`, `ai`, `archive`); generation_id?; created_by?; created_at | U(communication,version); U(generation_id); parent/generation RESTRICT; actor SET NULL. Immutable snapshots. |
...
Phase 15 `event_communication_generations` stores user/operation UUID, scoped event/communication FK, base revision, model/settings, encrypted input/result, exact optional Brand version, request state/timestamps and returned token facts. It reuses the AI provider contract without creating private Assistant conversations. See EVENT_COMMUNICATIONS.md for query indexes, lifecycle and queue rules.
