...
Status: **Phase 2 authentication completed and verified on 2026-09-01**. The user moved Google and SMS authentication from the proposed Phase 4 into this phase. RBAC was not in the requested Phase 2 scope and remains deferred. See the [Phase 2 report](phases/phase-02.md).
...
| **4 — Identity administration / remaining access control (re-plan before work)** | Role and permission management, account approval UI, Google unlink/number replacement and any remaining identity administration requested later | Permission package tables and related audit structures when approved | Re-review scope because Google login/link and SMS OTP login/enrollment were completed in Phase 2. Real provider smoke checks still require credentials. |
...
Do not install all optional packages, create all target schema tables, enable public registrations, build authentication/business modules, call OpenAI, or provision production infrastructure in Phase 1. Fortify/RBAC are Phase 2. The final schema design is reviewed now so that Phase 1 does not force later restructuring.
...
| Phase 4 | Google client/callback and allowed account policy; selected SMS provider, sender and test number | Test doubles are permitted for development; do not claim real OAuth/SMS functionality has been verified |
