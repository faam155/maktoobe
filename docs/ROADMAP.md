...
| **4 — Google and SMS authentication** | Socialite Google sign-in/link/unlink; verified-phone enrollment and SMS OTP login; uniform post-auth checks and future MFA integration point | social_accounts, otp_challenges, phone fields if not already present | OAuth state and unsafe account-link attempts denied; OTP replay/expiry/resend/attempt limits tested; disabled/unapproved accounts denied by every method; real provider smoke checks completed with configured test accounts |
