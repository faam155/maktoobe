...
Status: **working architecture baseline; Phase 13 Event Photos and File Management completed and verified on 2026-09-02**. Later phases remain subject to their scope review. See the [Phase 13 report](phases/phase-13.md) for implementation and verification.
...
## Phase 16 implementation refinement

The approved notification phase uses a durable workspace_notices ledger in domain Actions, followed by after-commit jobs, rather than requiring separate event/listener classes for each hook. This keeps notification intent atomic with existing mutations and allows scheduler recovery across a commit/enqueue interruption. Laravel Notifications remains the delivery abstraction. Database payloads contain references; current authorized resource titles and the current UI locale are resolved when reading. System notices retain both language versions. Any future external channel must choose and persist its delivery locale separately.

See NOTIFICATIONS.md and the Phase 16 report. Phases 10–15 subsequently implemented private Brand Guideline versions/context, event core/calendar, secure photos/files, report versions and event communication drafts/AI suggestions; those module documents supersede earlier planning-only assumptions. Phase 16 does not authorize analytics, API, MFA or external notification delivery.
